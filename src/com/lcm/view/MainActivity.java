...
//		int accountingDate = 25; // its value should come from setting
		...
		Log.e(TAG,"Update MonthlyData: from " + fromTo[Util.FROM] + " throughout " + fromTo[Util.THROUGH] + " to " + fromTo[Util.TO]);
		int[] expense = null;
		double maxExpense = 0;
...
//				(int)(remainingDays[0]*100/(remainingDays[0]+remainingDays[1])) + "% 지남");
		...
		remainedDetail = (trendBudget>0)? remainedDetail+df.format(trendBudget)+" 원 덜 사용" : remainedDetail+(df.format(-1*trendBudget))+" 원 더 사용";
		...
		trendDetail.setText("현 추세로 사용 시 말일에 " + df.format(expectation2) + "원 남음");
